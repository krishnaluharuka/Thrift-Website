	...
	function joyas_shop_setup() {
		...
		*/
		...
		if ( is_customize_preview() ) {
			...
			add_theme_support( 'starter-content', joyas_shop_get_starter_content() );
		}
...
function joyas_shop_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Homepage Banner', 'joyas-shop' ),
		'id'            => 'slider',
		'description'   => esc_html__( 'Add widgets here.', 'joyas-shop' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
...
function joyas_shop_scripts() {
	...
	$custom_css = ':root {--primary-color:'.esc_attr( get_theme_mod('__primary_color','#6c757d') ).'; --secondary-color: '.esc_attr( get_theme_mod('__secondary_color','#FF4343') ).'; --nav-h-color:'.esc_attr( get_theme_mod('__secondary_color','#FF4343') ).'}';
		
	wp_add_inline_style( 'joyas-shop-style', $custom_css );
...
function joyas_shop_register_block_pattern_categories()
{
	register_block_pattern_category(
		'joyas-shop',
		array(
			'label'       => __('Joyas shop', 'joyas-shop'),
		)
	);

}
add_action('init', 'joyas_shop_register_block_pattern_categories');
